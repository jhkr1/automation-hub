...
`google_finance`는 exchange-qualified symbol을 Playwright로 수집하고 `StockPrice`로 변환합니다. 선택적으로 MySQL snapshot을 저장하며, 저장된 최신 snapshot과 관련 Google News를 사용해 Movement와 StockInsight를 CLI에 출력합니다. Watchlist 분석은 가능한 symbol을 Batch로 처리하고 성공한 전체 결과를 profile별 JSON artifact로 저장합니다.
...
| `GEMINI_GOOGLE_FINANCE_API_KEY_PROD` | `--analyze --key-profile production` | production Gemini credential |
| `GEMINI_GOOGLE_FINANCE_API_KEY_TEST` | `--analyze --key-profile test` | 수동 smoke test Gemini credential |
| `GEMINI_MODEL` | 분석 | 선택적 Gemini model 이름. 비어 있으면 Runtime 기본값 사용 |
...
| `python -m google_finance.main AAPL:NASDAQ --analyze --key-profile production` | 저장된 변동과 Google News·Gemini 분석을 출력 |
| `python -m google_finance.watchlist_main --collect` | `STOCK_SYMBOLS`를 순차 수집·저장 |
| `python -m google_finance.watchlist_main --analyze --key-profile production` | Watchlist의 저장 snapshot을 Batch 분석하고 조건을 충족하면 production artifact 저장 |
...
- Google News RSS Provider, 단일 symbol Gemini 분석과 Watchlist Batch 분석
- 뉴스가 없을 때 Gemini를 호출하지 않는 근거 부족 결과
- `STOCK_SYMBOLS` 기반 순차 Watchlist 수집·분석과 profile별 Insight artifact 저장
- production artifact를 읽는 Google Finance Dashboard Insight
- Fake 기반 테스트, Google Finance CLI 검증과 MySQL Integration Test

단일 symbol `main.py --analyze` 결과는 CLI 출력만 합니다. Watchlist 분석은 `FAILED` 또는
`ANALYSIS_UNAVAILABLE` 결과가 없을 때만 JSON artifact를 원자적으로 저장하며, DB에는 분석
결과를 저장하지 않습니다.

| Profile | Artifact path | Dashboard 사용 |
|---|---|---|
| production | `output/google_finance_insights.json` | 예 |
| test | `output/test/google_finance_insights.json` | 아니오 |
...
- Scheduler, threshold, 상대 변동률, 분석 결과의 DB 저장은 현재 범위가 아닙니다.
